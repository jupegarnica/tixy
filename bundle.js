...
function scale(...args) {
  const f = (t) => f._at(t);
  Object.getOwnPropertyNames(Scale.prototype).forEach((key) =>
    f[key] = Scale.prototype[key]
  );
...
class Scale {
  ...
  domain(...domain) {
    ...
    if (2 == domain.length) {
      ...
      "function" !== typeof this._colors && domain.length == this._colors.length
    ) {
  ...
  _init(colorsOrFunction) {
    ...
    if ("function" != typeof colorsOrFunction) {
      this._pos = colorsOrFunction.map((_, i) =>
        i / (colorsOrFunction.length - 1)
      );
    }
    this._mode = "rgb";
    this.domain(0, 1);
...
function distance(a, b, mode = "lab") {
  ...
  const channelDifferences = l1.map((channelValue, channelIndex) =>
    channelValue - l2[channelIndex]
  );
...
function guess(args, mode) {
  ...
  if (!mode) {
    if (args.length == 1 && args[0] in w3cx11) {
      ...
      args.length == 4 && "number" == typeof args[3] && args[3] >= 0 &&
      args[3] <= 1
    ) {
      mode = "rgb";
    } else if (
      args.length == 1 && "number" == typeof args[0] && args[0] >= 0 &&
      args[0] <= 16777215
    ) {
      mode = "num";
    } else {
  ...
  return new Color(
    ...
    channels[1],
    channels[2],
    undefined !== channels[3] ? channels[3] : 1,
  );
}
...
function css2rgb(css) {
  ...
  if (m = css.match(CSS_RGB_REGEX) || css.match(CSS_RGB_WS_REGEX)) {
    ...
    m = css.match(CSS_RGB_PERCENT_REGEX) || css.match(CSS_RGB_WS_PERCENT_REGEX)
  ) {
    return [
      clamp(+m[1] / 100) * 255,
      clamp(+m[2] / 100) * 255,
...
function hsl2css([h, s, l], alpha) {
  ...
  return mode + "(" + rnd(h) + "," + rnd(s * 100) + "%" + "," + rnd(l * 100) +
    "%" + ("hsla" == mode ? "," + rnd(alpha) : "") + ")";
}
_input.css = css2rgb;
...
function kelvin2rgb(kelvin) {
  ...
  if (t < 66) {
    r = 255;
    g = -155.25485562709179 - 0.44596950469579133 * (t - 2) +
      104.49216199393888 * log(t - 2);
    b = t < 20
      ? 0
      : -254.76935184120902 + 0.8274096064007395 * (t - 10) +
        115.67994401066147 * log(t - 10);
  } else {
    r = 351.97690566805693 + 0.114206453784165 * (t - 55) -
      40.25366309332127 * log(t - 55);
    g = 325.4494125711974 + 0.07943456536662342 * (t - 50) -
      28.0852963507957 * log(t - 50);
    b = 255;
  }
  return [
    r,
...
function rgb2kelvin(r255, g255, b255) {
  ...
  if (g255 + b255 < 158.61) {
    ...
    return round(
      newtonIterate1d(
        (t) =>
          g255 -
          (-155.25485562709179 - 0.44596950469579133 * (t - 2) +
            104.49216199393888 * log(t - 2)),
        15,
        4,
      ) * 100,
    ...
    return (Math.E **
        ((g255 + 155.25485562709179 + 0.44596950469579133 * (15 - 2)) /
          104.49216199393888) + 2) * 100;
  } else if (b255 - r255 < 0) {
    console.log("20 < t < 66");
    return round(
      newtonIterate1d(
    ...
    return (Math.E **
        ((b255 + 254.76935184120902 - 0.8274096064007395 * (43 - 10)) /
          115.67994401066147) + 10) * 100;
  } else {
    ...
    return round(
      newtonIterate1d(
        (t) =>
          r255 -
          (351.97690566805693 + 0.114206453784165 * (t - 55) -
            40.25366309332127 * log(t - 55)),
        -1.4 * (r255 + g255) + 755,
        8,
      ) * 100,
  ...
  return newtonIterate1d(
    (k) => {
      ...
      const dkr = (kr2 - kr) / 0.000000001,
        dkg = (kg2 - kg) / 0.000000001,
        dkb = (kb2 - kb) / 0.000000001;
...
_input.hsi = hsi2rgb;
interpolators.hsv = interpolators.hsl = interpolators.hsi = interpolators.lch =
  interpolators.hcg = function interpolate_hsx(color1, color2, f, m) {
...
function render() {
  ...
  for (let y = 0; y < 16; y++) {
    for (let x = 0; x < 16; x++) {
      ...
      const fillStyle = typeof color1 == "string"
        ? color1
        : chooseColor(color1);
      context.beginPath();
      ...
      if (index === 0) {
        ...
        document.documentElement.style.setProperty(
          "--text-color",
          distance1 < 70 ? "#000" : defaultTextColor,
        );
        document.documentElement.style.setProperty(
          "--input-color",
          contrastedColor,
